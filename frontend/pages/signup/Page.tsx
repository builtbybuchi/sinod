...
const SignUpPage: React.FC<SignUpPageProps> = ({ onNavigate }) => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send signup request to Python backend
      ...
      if (response.data.success) {
        onNavigate(View.SIGN_UP_SUCCESS);
      } else {
      ...
      <main className="flex-1 flex flex-col">
        <div className="flex-1 w-full mx-auto max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-16 px-0 md:px-12 py-0 md:py-12 items-stretch">
          ...
          <div className="relative h-48 sm:h-56 md:h-64 lg:h-full w-full flex items-center justify-center order-1 lg:order-2 mb-10 lg:mb-0 overflow-visible">
            ...
            <div className="relative h-40 sm:h-48 md:h-56 lg:h-auto w-64 sm:w-72 md:w-80 lg:w-4/5 xl:w-full lg:aspect-[4/3] aspect-video rounded-xl lg:rounded-2xl overflow-hidden ring-1 ring-white/10 shadow-lg lg:shadow-2xl shadow-black/40 backdrop-blur-sm transition-all">
              <img
                ...
                alt="Team onboarding collaboration"
          ...
          <div className="max-w-md w-full mx-auto px-6 md:px-0 order-2 lg:order-1 pb-12 md:pb-0">
            <h1 className="text-3xl md:text-4xl text-center font-extrabold tracking-tight text-white mb-2">Create account</h1>
            {currentUser && !authLoading ? (
              <div className="text-center space-y-6 mt-10">
                <p className="text-gray-300 text-lg">You're already signed in!</p>
                <button 
                  onClick={() => onNavigate(View.DASHBOARD)} 
                  className="w-full inline-flex justify-center items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg shadow-md shadow-blue-600/20 transition-colors"
                >
                  Go to Dashboard
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input id="firstName" type="text" required value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full bg-gray-800/60 border border-gray-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/50 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 transition-colors" placeholder="Jane" />
                </div>
                <div>
                  <label htmlFor="lastName" className="block text-sm font-medium text-gray-300 mb-2">Last name</label>
                  <input id="lastName" type="text" required value={lastName} onChange={(e) => setLastName(e.target.value)} className="w-full bg-gray-800/60 border border-gray-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/50 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 transition-colors" placeholder="Doe" />
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="relative">
                  <input id="password" type={showPassword ? "text" : "password"} autoComplete="new-password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-gray-800/60 border border-gray-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/50 rounded-lg px-4 py-2.5 pr-12 text-white placeholder-gray-500 transition-colors" placeholder="Create a strong password" />
              ...
              <div className="flex items-start gap-3">
                <input id="tos" type="checkbox" required className="mt-1 h-4 w-4 rounded border-gray-600 bg-gray-800 text-blue-600 focus:ring-blue-500" />
                <label htmlFor="tos" className="text-xs text-gray-400 leading-relaxed">I agree to the <span className="text-blue-400">Terms</span> and <span className="text-blue-400">Privacy Policy</span>.</label>
              </div>
              <button type="submit" disabled={loading} className="w-full inline-flex justify-center items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg shadow-md shadow-blue-600/20 transition-colors disabled:opacity-70 disabled:cursor-not-allowed">
                ...
                */}
                <p className="text-gray-400 mt-10 text-sm">Already have an account?{' '}<button onClick={() => onNavigate(View.SIGN_IN)} className="text-blue-400 hover:text-blue-300 font-medium">Sign in</button></p>
              </>
            )}
          </div>
