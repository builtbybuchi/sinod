# Cloudflare Workers Deployment Guide for Backend (Python)

## Overview
Cloudflare Workers does not natively support Python. This guide outlines the available options for deploying the Python backend to Cloudflare.
...
Deploy your Python backend to Cloudflare Run and use Cloudflare Workers as a proxy/API gateway.

### Steps
1. Containerize your backend using the existing Dockerfile
...
export default {
  async fetch(request, env, ctx) {
    ...
    const backendUrl = `https://your-backend-run-url.workers.dev${url.pathname}${url.search}`;
...
If feasible, consider rewriting the backend in Node.js/TypeScript to fully leverage Cloudflare Workers.
...
- `R2_ACCESS_KEY_ID`
- `R2_ACCOUNT_ID`
- All other backend-specific secrets
